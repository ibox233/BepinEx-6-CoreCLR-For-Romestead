...
2. Extract this archive directly into the Romestead dedicated server folder.
   After extraction, install.bat should be next to Server.runtimeconfig.json.
3. Run install.bat from the server folder.
...
Run uninstall.bat from the server folder. It restores the runtimeconfig backup
...
- If Steam verifies server files or the server updates, the startup hook may be
  removed. Run install.bat again if BepInEx stops starting.
