...
if [ ! -f "$runtime_config" ]; then
    ...
    echo "Run ./uninstall.sh from the Romestead game directory."
    exit 1
fi
...
if hook_value.endswith("BepInEx.NET.CoreCLR.dll"):
    ...
    print("No Romestead BepInEx STARTUP_HOOKS entry found.")
PY
fi
...
echo ""
echo "Loader hook removed. The BepInEx folder was kept so user plugins/configs are not deleted."
