...
server_exe="$script_dir/Server"
server_dll="$script_dir/Server.dll"
...
if [ ! -f "$runtime_config" ]; then
    ...
    echo "Extract this archive directly into the Romestead dedicated server directory, then run sh install.sh again."
    exit 1
fi

if [ ! -f "$server_exe" ] && [ ! -f "$server_dll" ]; then
    echo "Server executable files were not found. Expected Server or Server.dll in: $script_dir"
    exit 1
fi
...
echo ""
echo "Romestead Server BepInEx mod loader installed."
